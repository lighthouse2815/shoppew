...
- Android push registration/revocation, encrypted FID storage, Firebase Admin provider selection, invalid-target deactivation, and bounded persisted retries are implemented. Production still needs a real Firebase project, least-privilege Application Default Credentials, client Firebase configuration, and physical-device delivery evidence.
- Notification delivery retry state is durable in PostgreSQL, but external send plus database commit cannot be globally atomic. A process failure after provider acceptance and before commit can cause an at-least-once duplicate; provider-specific idempotency/deduplication must be reviewed before promising exactly-once delivery.
